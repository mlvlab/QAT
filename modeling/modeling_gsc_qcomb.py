...
class GSC_Message_Passing(nn.Module):
    def __init__(self, k, n_ntype, n_etype, sent_dim, combiner, hidden_size, without_regulator, 
                 remove_qq, remove_aa, remove_qa, remove_za, remove_zq):
        super().__init__()
        ...
        if self.combiner == 'mult' :
            self.sent_encoder = MLP(sent_dim, hidden_size, n_etype, 1, 0)
            ...
            self.sent_encoder = nn.Sequential(
                nn.Linear(sent_dim, hidden_size),
                nn.LayerNorm(hidden_size)
            )
            self.edge_encoder = nn.Sequential(
                nn.Linear(n_etype + n_ntype *2, hidden_size),
                nn.LayerNorm(hidden_size)
            )
        ...
        elif self.combiner == 'innerprod' :
            self.sent_encoder = MLP(sent_dim, hidden_size, sent_dim, 1, 0)
            self.edge_encoder = MLP(n_etype+ n_ntype *2, hidden_size, sent_dim, 1, 0, layer_norm=True)
        elif self.combiner == 'attn' :
            self.sent_encoder = MLP(sent_dim, hidden_size, sent_dim, 1, 0)
            self.edge_encoder = MLP(n_etype+ n_ntype *2, hidden_size, sent_dim, 1, 0, layer_norm=True)
            self.edge_encoder2 = nn.Sequential(MLP(sent_dim, hidden_size, 1, 1, 0, layer_norm=True), nn.Sigmoid())
        elif self.combiner in ('jy1', 'jy2'):
            self.sent_encoder = MLP(sent_dim, hidden_size, sent_dim, 1, 0)
            self.edge_encoder = MLP(n_etype+n_ntype*2, hidden_size, sent_dim, 1, 0, layer_norm=True)
        else :
            raise ValueError("check qcombiner value")
        self.k = k
    ...
    def get_graph_edge_embedding(self, edge_index, edge_type, node_type_ids, sent_embeddings, unflattened_edge_type):
        ...
        if self.combiner == 'mult' :
            context_embs = []
            for i, entity in enumerate(unflattened_edge_type) :
                import pdb; pdb.set_trace()
                edge_oh = make_one_hot(entity, self.n_etype)
                context_emb = torch.matmul(edge_oh, sent_embeddings[i])
                context_embs.append(context_emb)
            context_embeddings = torch.cat(context_embs, 0).unsqueeze(-1)
            edge_embeddings = context_embeddings * edge_embeddings
            info = edge_embeddings.data
        elif self.combiner == 'concat' :
            sent_embs = []
            for i, entity in enumerate(unflattened_edge_type) :
                sent_embs.append(sent_embeddings[i].unsqueeze(0).repeat(entity.shape[0],1))
            z = self.activation(torch.cat([torch.cat(sent_embs, 0), edge_embeddings],1))
            edge_embeddings = self.edge_encoder2(z)
            info = edge_embeddings.data
        else :
            context_embs = []
            for i, entity in enumerate(unflattened_edge_type) :
                context_embs.append(sent_embeddings[i].unsqueeze(0).repeat(entity.shape[0],1))
            context_embeddings = torch.cat(context_embs, 0)
            if self.combiner == 'innerprod' :
                edge_embeddings = (context_embeddings * edge_embeddings).sum(-1, keepdim=True).sigmoid()
                info = edge_embeddings.data
            elif self.combiner == 'attn' :
                A = (context_embeddings * edge_embeddings).sum(-1, keepdim=True).softmax(dim=-1)
                edge_embeddings = A * self.edge_encoder2(edge_embeddings)
                info = A.data
    ...
    def forward(self, adj, node_type_ids, sent_vecs, unflattened_edge_type):
        ...
        sent_embeddings = self.sent_encoder(sent_vecs)
        edge_embeddings, info = self.get_graph_edge_embedding(edge_index, edge_type, node_type_ids, sent_embeddings, unflattened_edge_type)
        ...
        return aggr_out, (edge_index, edge_score_list, info)
...
class QAGSC(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim, enc_dim,
        ...
        if not self.without_gnn:
            self.gnn = GSC_Message_Passing(k, n_ntype, n_etype, sent_dim=sent_dim, combiner=args.qcombine,
                                            hidden_size=enc_dim, without_regulator=args.without_regulator, 
                                            remove_qq=args.remove_qq, remove_aa=args.remove_aa, remove_qa=args.remove_qa, 
                                            remove_za=args.remove_za, remove_zq=args.remove_zq)
        self.fc = MLP(sent_dim, fc_dim, 1, n_fc_layer, p_fc, layer_norm=True)


    def forward(self, sent_vecs, concept_ids, node_type_ids, unflattened_edge_type, adj, detail=False):
        context_score = self.fc(sent_vecs)
        if not self.without_gnn:
            graph_score, infos = self.gnn(adj, node_type_ids, sent_vecs, unflattened_edge_type)   #(batch_size, dim_node)
...
class LM_QAGSC_QCOMB(nn.Module):
    ...
    def forward(self, *inputs, detail=False):
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        ...
        if detail:
            logits, infos = self.decoder(sent_vecs.to(node_type_ids.device), concept_ids, node_type_ids, unflattened_edge_type, adj, detail=detail)
        else:
            logits = self.decoder(sent_vecs.to(node_type_ids.device), concept_ids, node_type_ids, unflattened_edge_type, adj)
