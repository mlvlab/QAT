...
class FullTransformer(nn.Module):
    ...
    def __init__(self, layer_num, n_ntype, n_etype, d_sentence, d_model, nhead, num_lmtokens, dim_feedforward=2048, 
                 ...
                 add_textfeatsim=False, add_relativeposemb=False, use_windowattention=False, use_wandb=False, encoder_type='roberta-large', add_qaedge=False, epsilon=1e-8, data=None, rpe_2=False, drop_ratio=1.0):
    ...
    def construct_token(self, edge_embeddings, unflattened_edge_type, node_emb, node_mask, matched=None) :
        """construct token
        ...
        if self.add_relativeposemb:
            return tokens, masks, e_num, v_num, matched_tokens
        else:
            return tokens, masks, e_num, v_num
    ...
    def forward(self, adj, sent_vecs, node_type_ids, unflattened_edge_type, lm_all_states, lm_mask, textfeat,
        ...
        if self.add_relativeposemb:
            ...
            graph_tokens, graph_mask, e_num, v_num, matched_tokens = self.construct_token(edge_embeddings, unflattened_edge_type, node_emb, node_mask, matched)
        else:
            edge_embeddings = self.get_graph_edge_embedding(edge_index, edge_type, node_type_ids, node_emb, lm_all_states, textfeat, qids)
            graph_tokens, graph_mask, e_num, v_num = self.construct_token(edge_embeddings, unflattened_edge_type, node_emb, node_mask, matched=None)
