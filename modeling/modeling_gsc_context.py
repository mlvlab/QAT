...
class QAGSC(nn.Module):
    def __init__(self, args, k, n_ntype, n_etype, sent_dim, 
        ...
        if args.use_nodeemb:
            ...
            self.attn = CQAttention(block_hidden_dim=concept_dim)
    ...
    def forward(self, sent_vecs, concept_ids, node_type_ids, adj_lengths, adj, detail=False, emb_data=None, hidden_states=None, context_mask=None):
        ...
        if not self.without_gnn:
            if self.use_nodeemb:
                ...
                # node_emb = self.dropout_e(torch.cat((gnn_input0, gnn_input1), dim=1))
                node_emb = torch.cat((gnn_input0, gnn_input1), dim=1)
                gnn_mask = torch.arange(node_type_ids.size(1), device=node_type_ids.device) < adj_lengths.unsqueeze(1) #(B, M)
                hidden_states = self.svec2nvec(hidden_states)
                if self.nodeemb_type == "type1":
                    attention = self.attn(node_emb, hidden_states, gnn_mask, context_mask)
                    gnn_input = self.dropout_e(attention@hidden_states)

                    graph_score, infos = self.gnn(adj, node_type_ids, node_emb=gnn_input)
                    infos.append(attention)
                elif self.nodeemb_type == "type2":
                    attention = self.attn(hidden_states, node_emb, context_mask, gnn_mask)
...
class LM_QAGSC(nn.Module):
    ...
    def forward(self, *inputs, detail=False):
        ...
        *lm_inputs, concept_ids, node_type_ids, node_scores, adj_lengths, edge_index, edge_type = _inputs
        sent_vecs, hidden_states = self.encoder(*lm_inputs)
        ...
        if detail:
            logits, infos = self.decoder(sent_vecs.to(node_type_ids.device), concept_ids, node_type_ids, adj_lengths, adj, detail=detail, emb_data=None, hidden_states=hidden_states.to(node_type_ids.device), context_mask=(~lm_inputs[3]).to(node_type_ids.device))
        else:
            logits = self.decoder(sent_vecs.to(node_type_ids.device), concept_ids, node_type_ids, adj_lengths, adj, emb_data=None, hidden_states=hidden_states.to(node_type_ids.device), context_mask=(~lm_inputs[3]).to(node_type_ids.device))
